__global__ void cuda_reduction_6(I *z, I *x, I total, I bound){
    ...
    UI tid = threadIdx.x;
    UI blockSize = blockDim.x;
    UI i = blockIdx.x * blockSize * 2 + tid;
    UI gridSize = gridDim.x * (blockSize * 2);
    sharedData[tid] = 0;
...
static I run_gpu_v6(I *x, L n){
    I *dx, *dz, *z;
    I numThread = NUM_THREAD;
    I numBlock  = (n/numThread/2) + (n%numThread!=0); /* numBlock should be halved */
    ...
    cuda_reduction_6<<<numBlock, numThread, memSize>>>(dz, dx, numBlock * 2 * numThread, n);
    cudaDeviceSynchronize(); /* Wait for compute device to finish */
    CUDA_SAVE(z, dz, F, numBlock);
    I rtn = calcSum(z, numBlock);
    gettimeofday(&t2, 0);
    P("[GPU] The elapsed average time (ms): %g (v6)\n", calcTime(t1,t2));
    DOI(5, P("z[%lld] = %d\n", i,z[i]))
    CUDA_FREE(dx);
    CUDA_FREE(dz);
    R rtn;
